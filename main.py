...
secret_word = words.LIST[random.randint(0, len(words.LIST) - 1)]
game_over = False
letters = 0
turn_active = True
...
def draw_board():
    ...
    for col in range(0, 5):
        for row in range(0, 6):
            ...
            piece_text = title_font.render(board[row][col], True, grey)
...
while running:
    ...
    if turn == 6 and not guess == secret_word:
        game_over = True
        game_over_text = title_font.render('Loser!', True, red)
        screen.blit(game_over_text, (160, 610))
        secret_word_display = small_font.render(secret_word, True, white)
        screen.blit(secret_word_display, (210, 660))

    if turn == 6 and guess == secret_word:
        winner6_text = title_font.render('Winner!', True, pink)
        screen.blit(winner6_text, (155, 610))

    if game_over and turn < 6:
        winner_text = title_font.render('Winner!', True, pink)
        screen.blit(winner_text, (155, 610))
